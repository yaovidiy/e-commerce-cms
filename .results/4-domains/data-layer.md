...
// With where clause
const filtered = await db.select().from(table).where(eq(table.id, 'value'));

// With joins
const joined = await db.select().from(table1).innerJoin(table2, eq(table1.foreignKey, table2.id));
```
...
where(or(eq(table.field, value1), eq(table.field, value2)));
```
