<script lang="ts">
	import { createMegaMenu, getAllMegaMenus } from '$lib/remotes/mega-menu.remote';
	...
	let form = $state(createMegaMenu);
	let selectedCategoryIds = $state<string[]>([]);
	let showCategoryBrowser = $state(false);
	let selectedMainCategory = $state('');
...
>
	...
	<Button type="submit" disabled={!!form.pending} class="w-full">
		{form.pending ? m.common_saving() : m.common_create()}
	</Button>
</form>
