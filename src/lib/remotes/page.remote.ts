...
export const getAllPages = query(FilterPagesSchema, async (filters) => {
	...
	if (sortField === 'title') {
		query = query.orderBy(
			sortDirection === 'asc' ? asc(tables.page.title) : desc(tables.page.title)
		) as typeof query;
	} else if (sortField === 'status') {
		query = query.orderBy(
			sortDirection === 'asc' ? asc(tables.page.status) : desc(tables.page.status)
		) as typeof query;
	} else if (sortField === 'createdAt') {
		query = query.orderBy(
			sortDirection === 'asc' ? asc(tables.page.createdAt) : desc(tables.page.createdAt)
		) as typeof query;
	} else if (sortField === 'publishedAt') {
		query = query.orderBy(
			sortDirection === 'asc' ? asc(tables.page.publishedAt) : desc(tables.page.publishedAt)
		) as typeof query;
	} else {
		query = query.orderBy(
			sortDirection === 'asc' ? asc(tables.page.updatedAt) : desc(tables.page.updatedAt)
		) as typeof query;
	}
...
export const createPage = form(CreatePageSchema, async (data) => {
	...
	const [existing] = await db.select().from(tables.page).where(eq(tables.page.slug, data.slug));
...
export const updatePage = form(UpdatePageSchema, async (data) => {
	...
	const [existing] = await db.select().from(tables.page).where(eq(tables.page.id, data.id));
	...
	if (data.slug && data.slug !== existing.slug) {
		const [slugExists] = await db.select().from(tables.page).where(eq(tables.page.slug, data.slug));
...
export const deletePage = form(DeletePageSchema, async (data) => {
	...
	const [page] = await db.select().from(tables.page).where(eq(tables.page.id, data.id));
